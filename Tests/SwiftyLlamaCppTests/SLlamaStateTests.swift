...
struct SLlamaStateTests {
    ...
    func sLlamaStateConstruction() throws {
    ...
    func sLlamaStateDataOperations() throws {
        let executed = TestUtilities.withDummyContext { ctx in
            let state = SLlamaState(context: ctx)
    ...
    func sLlamaStateFileOperations() throws {
        let executed = TestUtilities.withDummyContext { ctx in
            let state = SLlamaState(context: ctx)
    ...
    func sLlamaStateSequenceOperations() throws {
        let executed = TestUtilities.withDummyContext { ctx in
            let state = SLlamaState(context: ctx)
    ...
    func sLlamaContextStateExtension() throws {
        let executed = TestUtilities.withDummyContext { ctx in
            let state = ctx.state()
            #expect(state.getStateSize() == 0)
        }
    ...
    func sLlamaContextStateConvenienceMethods() throws {
        let executed = TestUtilities.withDummyContext { ctx in
            // Test state size
    ...
    func sLlamaContextStateConvenienceDataMethods() throws {
        let executed = TestUtilities.withDummyContext { ctx in
